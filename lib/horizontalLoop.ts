...
// Ported from GSAP's official "horizontalLoop" helper function recipe.
// Builds an infinite, draggable, snap-to-slide horizontal timeline out of
// arbitrary DOM elements. Kept close to the reference implementation since
// it's a dense, well-tested closure-heavy algorithm — typed loosely on
// purpose rather than fighting TS over its dynamic internals.
...
export function horizontalLoop(items: Element[], config: HorizontalLoopConfig = {}): HorizontalLoopTimeline {
  ...
  const itemsArr = gsap.utils.toArray(items) as HTMLElement[];
  ...
  gsap.context(() => {
    ...
    let timeOffset = 0;
    const container: any =
      center === true ? itemsArr[0].parentNode : (gsap.utils.toArray(center as any)[0] as any) || itemsArr[0].parentNode;
    ...
    const populateWidths = () => {
      ...
      gsap.set(itemsArr, { xPercent: (i: number) => xPercents[i] });
    ...
    if (config.draggable && typeof Draggable === "function") {
      ...
      let ratio: number, startProgress: number, lastSnap: number, initChangeX: number, wasPlaying: boolean;
      // eslint-disable-next-line prefer-const -- `align` closes over this before it's assigned below.
      let draggable: any;
      const align = () => tl.progress(wrap(startProgress + (draggable.startX - draggable.x) * ratio));
      const syncIndex = () => tl.closestIndex(true);

      draggable = Draggable.create(proxy, {
        trigger: itemsArr[0].parentElement as Element,
        type: "x",
        onPressInit() {
          const x = this.x;
    ...
    tl.kill = (...args: unknown[]) => {
      window.removeEventListener("resize", onResize);
      tl.draggable?.kill?.();
      return originalKill(...(args as []));
    };

    timeline = tl;
  });

  // @ts-expect-error assigned inside gsap.context synchronously above
  return timeline;
}
